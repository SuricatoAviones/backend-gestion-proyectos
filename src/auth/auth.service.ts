...
export class AuthService {
  ...
  async login({ in_usuario, password }: LoginDto) {
    ...
    const payload = {
      id: user.i001i_usuario,
      email: user.in_correo,
      role: user.in_role,
      nombre: user.in_nombre,
      apellido: user.in_apellido,
      foto: user.foto,
      usuario: user.in_usuario,
    };
  ...
  async profile({ in_correo }: { in_correo: string; role: string }) {
